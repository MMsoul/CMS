			...
			<div class="col-md-2 ">
				<ul class="list-group list-group-flush">
					<li class="list-group-item list-group-item-action text-center"><a
						class="channel active" href="/article/1">热门</a></li>
					...
					<c:forEach items="${_CHANNELS_AND_CATEGORIES_}" var="c">
						<c:forEach items="${c}" var="channel">
							<li class="list-group-item list-group-item-action text-center">
								<!-- 将频道的名称作为超级连接，当单击它时，会再次请求主页的显示，同时，向后台传递频道的ID --> <a
								class="channel" href="/?channel=${channel.key.id}">${channel.key.name}</a>
							</li>
						</c:forEach>
					</c:forEach>
				</ul>
								...
								<c:forEach items="${channels }" var="c">
									<c:if test="${c.key.id eq channel.id}">
										...
										<c:forEach items="${c.value}" var="cate">
											<li class="nav-item"><a
												class="nav-link ${cate.id==category ? 'active' : ''}"
												href="/?channel=${c.key.id}&category=${cate.id}">${cate.name}</a>
											</li>
										</c:forEach>
									</c:if>
								</c:forEach>
				...
				<c:forEach items="${articles}" var="article">
					<c:choose>
						...
						<c:otherwise>
							<div>
								<h3>
									<a class="article_title" href="/article/${article.id}">${article.title }</a>
								</h3>
								<p>${article.summary}</p>
								<p class="blog_item_footer">
									<span class="glyphicon glyphicon-user" title="作者"></span>作者：${article.author.nickname}&nbsp;&nbsp;&nbsp;
			...
			<div class="col-md-3">
				<div class="card">
					...
					<div class="card-body">
						<ol>
							<c:forEach items="${lastArticles}" var="article">
								<li class="text-truncate"><a href="/article/${article.id}">
									<img class="d-block w-100" src="${article.picture}"
								</a></li>
							</c:forEach>
						</ol>
					</div>
